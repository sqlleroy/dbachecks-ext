    ...
    Set-PSFConfig -Module dbachecks -Name global.notcontactable -Value $NotContactable 
        ...
        if ($NotContactable -contains $psitem) {
            ...
            Context "Testing database collation on $psitem" {
                $fix = {
                    ...
                    $db.Collation = $TargetCollation
                    $db.Alter()
                }
                $testCases = Test-DbaDbCollation -SqlInstance $psitem -Database $Database -ExcludeDatabase $exclude | % { 
                    @{ 
                        InputObject = $_
                        InstanceObject = $connectioncheck
                        DatabaseCollation = $_.DatabaseCollation
                        ServerCollation = $_.ServerCollation
                        Database = $_.Database
                ...
                It -TestCases $testCases "database collation <DatabaseCollation> should match server collation <ServerCollation> for <Database> on <SqlInstance>" {
                    Param (
                        $InputObject, $InstanceObject, $Fix
                    )
                    $InputObject.ServerCollation | Should -Be $InputObject.DatabaseCollation -Because "You will get collation conflict errors in tempdb"
                }
                if ($Wrongcollation) {
        ...
        Describe "Suspect Page" -Tags SuspectPage, $filename {
            if ($NotContactable -contains $psitem) {
                ...
                Context "Testing suspect pages on $psitem" {
                    @(Connect-DbaInstance -SqlInstance $psitem).Databases.Where{if ($Database) {$_.Name -in $Database}else {$ExcludedDatabases -notcontains $PsItem.Name}}.ForEach{
