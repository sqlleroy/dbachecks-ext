    ...
    Describe "SQL Engine Service" -Tags SqlEngineServiceAccount, ServiceAccount, $filename {
        if ($NotContactable -contains $psitem) {
            ...
            Context "Testing SQL Engine Service on $Psitem. " {
                ...
                # ------------------------------------------------ #
                $checkBlock = {
                    $_.State | Should -Be $ReferenceValue -Because 'If the service is not running, the SQL Server will not be accessible'
                }
                # ----------------------------------------------- #
                # Current state of a setting to be tested/checked #
                # ----------------------------------------------- #
                If ($CurrentConfig = Get-DbaService -ComputerName $Server -InstanceName $InstanceName -Type Engine -ErrorAction SilentlyContinue) {
                    # --------------------------------------------------------------------- #
                    ...
                    # ---------------------- #
                    It @TestCases "SQL Engine service account should Be running on <InstanceName>"                
                    ...
                    if ($IsClustered) {
                        ...
                        # Pester check execution #
                        # ---------------------- #
                        It @TestCases  "SQL Engine service account should have a start mode of Manual on FailOver Clustered Instance <InstanceName>"                         
                    }
                    else {
                        # --------------------------------------- #
                        # Change the setting to the desired value #
                        ...
                        # ------------------------------------------------ #
                        $checkBlock = {
                            $_.StartMode | Should -Be $ReferenceValue -Because 'Otherwise If the server restarts, the SQL Server will not be accessible'
                        }
                        # --------------------------------------------------------------------- #
                        # Function Get-DbcTestCase formatting the expected output for TestCases #
                        # --------------------------------------------------------------------- #
                        ...
                        # Pester check execution #
                        # ---------------------- #
                        It @TestCases "SQL Engine service account should have a start mode of Automatic on standalone instance <InstanceName>"                       
                    }
                }
            }
        }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU7Iwn05PtOJEJFPqEvF3bY/NG
# 6HGgggpSMIIFGjCCBAKgAwIBAgIQAsF1KHTVwoQxhSrYoGRpyjANBgkqhkiG9w0B
...
# BgorBgEEAYI3AgEVMCMGCSqGSIb3DQEJBDEWBBQhd6UbAfGYR3o6h/0MBIH6hRDb
# nTANBgkqhkiG9w0BAQEFAASCAQAZBxwhK2rE1qR5mSj4bdRst4kIOFF+SFHc6TGy
# nWGibJ17LzCAHf7HDsBKs8ICl30xIXQ01ftXK0VFzqjmME+i/9qpVkWRzFNqCPGJ
# WtZ2qdNkKLgIiSeicVkou9NcVW68qsEVG/1UJXFBdCR2jAQfMz71qoVZafDVlY1G
# ZXoA3HSm/w6NF/b9VU7pvgTPfKpP6Bj0VLIqaOdQJbFlC2s3wHJiPuXS5Yb0v/wy
# DTLaedN8QVU+0phSGNPUfqdLdADaBnE7013qWNrNmp6EeD8HA0fmBIgyPFK5IX+L
# ZT/TSaKKuqYEYQM0++KtLQwIPlDwmo8LxpsoXLokNobsOwDH
